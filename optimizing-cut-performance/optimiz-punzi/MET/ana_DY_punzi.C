...
void ana_DY_punzi(const char *scanMet_step = "tmp", TString outputfile = "./output/tmp.root")
{
    ...
    for (int evt = 0; evt < T_inclusive->GetEntries(); evt++)
    {
        ...
        // Scan Met
        //-------------------
        ee_for_inclusive_sample(HT, atoi(scanMet_step), merge_weight, I_ht0_weight, h_pass_Bg);
        if (f_ht0_Met > metcut)
        {
            ee_for_inclusive_sample(HT, atoi(scanMet_step), merge_weight, I_ht0_weight, h_pass_Bg_nMetcut);
        }
    }
    TTree *T_HT100;
    ...
    for (int evt = 0; evt < T_HT100->GetEntries(); evt++)
    {
        ...
        // Scan Met
        //-------------------
        h_pass_Bg->Fill(atoi(scanMet_step), I_ht100_weight * merge_weight.ee_HT100Weight);
        if (f_ht100_Met > metcut)
        {
            h_pass_Bg_nMetcut->Fill(atoi(scanMet_step), I_ht100_weight * merge_weight.ee_HT100Weight);
        }
    }
    TTree *T_HT200;
    ...
    for (int evt = 0; evt < T_HT200->GetEntries(); evt++)
    {
        ...
        // Scan Met
        //-------------------
        h_pass_Bg->Fill(atoi(scanMet_step), I_ht200_weight * merge_weight.ee_HT200Weight);
        if (f_ht200_Met > metcut)
        {
            h_pass_Bg_nMetcut->Fill(atoi(scanMet_step), I_ht200_weight * merge_weight.ee_HT200Weight);
        }
    }
    TTree *T_HT400;
    ...
    for (int evt = 0; evt < T_HT400->GetEntries(); evt++)
    {
        ...
        // Scan Met
        //-------------------
        h_pass_Bg->Fill(atoi(scanMet_step), I_ht400_weight * merge_weight.ee_HT400Weight);
        if (f_ht400_Met > metcut)
        {
            h_pass_Bg_nMetcut->Fill(atoi(scanMet_step), I_ht400_weight * merge_weight.ee_HT400Weight);
        }
    }
    TTree *T_HT600;
    ...
    for (int evt = 0; evt < T_HT600->GetEntries(); evt++)
    {
        ...
        // Scan Met
        //-------------------
        h_pass_Bg->Fill(atoi(scanMet_step), I_ht600_weight * merge_weight.ee_HT600Weight);
        if (f_ht600_Met > metcut)
        {
            h_pass_Bg_nMetcut->Fill(atoi(scanMet_step), I_ht600_weight * merge_weight.ee_HT600Weight);
        }
    }
    TTree *T_HT800;
    ...
    for (int evt = 0; evt < T_HT800->GetEntries(); evt++)
    {
        ...
        // Scan Met
        //-------------------
        h_pass_Bg->Fill(atoi(scanMet_step), I_ht800_weight * merge_weight.ee_HT800Weight);
        if (f_ht800_Met > metcut)
        {
            h_pass_Bg_nMetcut->Fill(atoi(scanMet_step), I_ht800_weight * merge_weight.ee_HT800Weight);
        }
    }
    TTree *T_HT1200;
    ...
    for (int evt = 0; evt < T_HT1200->GetEntries(); evt++)
    {
        ...
        // Scan Met
        //-------------------
        h_pass_Bg->Fill(atoi(scanMet_step), I_ht1200_weight * merge_weight.ee_HT1200Weight);
        if (f_ht1200_Met > metcut)
        {
            h_pass_Bg_nMetcut->Fill(atoi(scanMet_step), I_ht1200_weight * merge_weight.ee_HT1200Weight);
        }
    }
    TTree *T_HT2500;
    ...
    for (int evt = 0; evt < T_HT2500->GetEntries(); evt++)
    {
        ...
        // Scan Met
        //-------------------
        h_pass_Bg->Fill(atoi(scanMet_step), I_ht2500_weight * merge_weight.ee_HT2500Weight);
        if (f_ht2500_Met > metcut)
        {
            h_pass_Bg_nMetcut->Fill(atoi(scanMet_step), I_ht2500_weight * merge_weight.ee_HT2500Weight);
        }
    }
