...
void ana_Triboson_bg(TString outputfile = "Triboson_output.root")
{
    //---------------------
    // dilepton cut
    //---------------------
    ...
    for (int evt = 0; evt < T_tree->GetEntries(); evt++)
    {
        ...
        h_pass_bg->Fill(1, I_WWZ_weight * WWZWeight);
        //-------------------
        // Scan dilepton PT
        //-------------------
        if (f_WWZ_dilepPT > dilepton_cut)
        {
            h_pass_Bg_nDilepPTcut->Fill(1, I_WWZ_weight * WWZWeight);
        }
        else
        {
            h_pass_Bg_nDilepPTcut->Fill(2, I_WWZ_weight * WWZWeight);
        }
    }
    TTree *T_tree2;
    ...
    for (int evt = 0; evt < T_tree2->GetEntries(); evt++)
    {
        ...
        h_pass_bg->Fill(1, I_WZZ_weight * WZZWeight);
        //-------------------
        // Scan dilepton PT
        //-------------------
        if (f_WZZ_dilepPT > dilepton_cut)
        {
            h_pass_Bg_nDilepPTcut->Fill(1, I_WZZ_weight * WZZWeight);
        }
        else
        {
            h_pass_Bg_nDilepPTcut->Fill(2, I_WZZ_weight * WZZWeight);
        }
    }
    TTree *T_tree3;
    ...
    for (int evt = 0; evt < T_tree3->GetEntries(); evt++)
    {
        ...
        h_pass_bg->Fill(1, I_ZZZ_weight * ZZZWeight);
        //-------------------
        // Scan dilepton PT
        //-------------------
        if (f_ZZZ_dilepPT > dilepton_cut)
        {
            h_pass_Bg_nDilepPTcut->Fill(1, I_ZZZ_weight * ZZZWeight);
        }
                else
        {
            h_pass_Bg_nDilepPTcut->Fill(2, I_ZZZ_weight * ZZZWeight);
        }
    }
