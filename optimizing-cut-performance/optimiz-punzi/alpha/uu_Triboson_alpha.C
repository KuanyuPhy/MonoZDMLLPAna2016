...
void uu_Triboson_alpha(const char *scanalpha_step = "tmp", TString outputfile = "./output/tmp.root")
{
    ...
    for (int evt = 0; evt < T_tree->GetEntries(); evt++)
    {
        ...
        {
            // h_Bg_alpha->Fill((*v_WWZ_alpha)[i], I_WWZ_weight * merge_weight.uu_WWZWeight);
        ...
        h_Bg_nJet->Fill(jet_passalpha_cut, I_WWZ_weight * merge_weight.uu_WWZWeight);

        if (jet_passalpha_cut < 2)
        {
            continue;
        }
        h_Bg_nJet_cuted->Fill(jet_passalpha_cut, I_WWZ_weight * merge_weight.uu_WWZWeight);
    }
    TTree *T_tree2;
    ...
    for (int evt = 0; evt < T_tree2->GetEntries(); evt++)
    {
        ...
        {
            // h_Bg_alpha->Fill((*v_WZZ_alpha)[i], I_WZZ_weight * merge_weight.uu_WZZWeight);
        ...
        h_Bg_nJet->Fill(jet_passalpha_cut, I_WZZ_weight * merge_weight.uu_WZZWeight);

        if (jet_passalpha_cut < 2)
        {
            continue;
        }
        h_Bg_nJet_cuted->Fill(jet_passalpha_cut, I_WZZ_weight * merge_weight.uu_WZZWeight);
    }
    ...
    for (int evt = 0; evt < T_tree3->GetEntries(); evt++)
    {
        ...
        {
            // h_Bg_alpha->Fill((*v_ZZZ_alpha)[i], I_ZZZ_weight * merge_weight.uu_ZZZWeight);
        ...
        h_Bg_nJet->Fill(jet_passalpha_cut, I_ZZZ_weight * merge_weight.uu_ZZZWeight);
        if (jet_passalpha_cut < 2)
        {
            continue;
        }
        h_Bg_nJet_cuted->Fill(jet_passalpha_cut, I_ZZZ_weight * merge_weight.uu_ZZZWeight);
    }
...
int main(int argc, char *argv[])
{
    if (argc == 3)
    {
        uu_Triboson_alpha(argv[1], argv[2]);
    }
    else
    {
        cout << "ERROR" << endl;
